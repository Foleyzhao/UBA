						...
						<div :key="positionInfo" v-for="(positionInfo, index) in formData.positionJson">
							<a-row :gutter="10">
								...
								<a-col :span="7">
									...
									>
										<sys-page-select
											ref="sysChildPositionPageSelectRef"
								...
								<a-col :span="3" class="sys-mt4">
									<a-button size="small" type="primary" danger ghost @click="delDomains(index)">移除
									</a-button>
								</a-col>
							</a-row>
						</div>
						...
						<a-col :span="12">
							<a-form-item label="民族：" name="nation">
								<a-select v-model:value="formData.nation" placeholder="请选择民族"
										  :options="nationOptions"></a-select>
							</a-form-item>
						</a-col>
						<a-col :span="12">
					...
					<a-row :gutter="16">
						<a-col :span="12">
							<a-form-item label="证件类型：" name="idCardType">
								<a-select v-model:value="formData.idCardType" placeholder="请选择证件类型"
						...
						<a-col :span="12">
							<a-form-item label="证件号码：" name="idCardNumber">
								<a-input v-model:value="formData.idCardNumber" placeholder="请输入证件号码"
										 allow-clear/>
							</a-form-item>
						</a-col>
					</a-row>
					<a-row :gutter="16">
						...
						<a-col :span="12">
							<a-form-item label="政治面貌：" name="politicalOutlook">
								<a-input v-model:value="formData.politicalOutlook" placeholder="请输入政治面貌"
										 allow-clear/>
							</a-form-item>
						</a-col>
					</a-row>
...
const sysUserPageSelectRef = ref()
const sysChildPositionPageSelectRef = ref()
const sysChildUserPageSelectRef = ref()
// 表单数据
const formData = ref({})
...
const childOrgSelect = async (data, type, index) => {
	...
	nextTick(() => {
		sysChildPositionPageSelectRef.value[index].onPage(param)
		sysChildUserPageSelectRef.value[index].onPage(param)
	})
}
// 验证并提交数据
const onSubmit = () => {
