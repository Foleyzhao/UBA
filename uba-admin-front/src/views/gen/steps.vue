	...
	<div>
		<a-card class="steps-card" :bordered="false">
			<a-row class="sys-row">
				...
				<a-col :span="6">
					<div class="sys-fdr">
						<a-button :disabled="current === 0" class="sys-ml8" @click="prev"> 上一步</a-button>
						<a-button :disabled="current === 2" type="primary" class="sys-ml8" @click="next"> 继续</a-button>
						<a-button type="primary" danger ghost class="sys-ml8" @click="emit('closed')"> 关闭</a-button>
					</div>
				</a-col>
			</a-row>
		</a-card>
		...
		<div v-if="current === 2">
			<a-card>
				<a-result status="success" title="操作成功" sub-title="此刻可预览代码，同时您可以一键生成代码啦">
					<template #extra>
						<a-space size="middle">
							<a-button v-if="current > 0" class="sys-ml8" @click="genPreviewRef.onOpen(recordData)">预览
							</a-button>
							<a-button
