			...
			<div class="login-form">
				<a-card>
						...
						<a-tab-pane key="userAccount" tab="账号密码">
							<a-form ref="loginForm" :model="ruleForm" :rules="rules">
								...
								<a-form-item name="account">
									<a-input v-model:value="ruleForm.account" placeholder="请输入账号" size="large"
											 @keyup.enter="login">
										<template #prefix>
								...
								<a-form-item>
									<div style="display: flex; justify-content: space-between">
										<a href="/front/client/findPwd" class="sys-color-0d84ff">忘记密码？</a>
										<a href="/front/client/register" class="sys-color-0d84ff"
										   v-if="registerOpen === 'true'">
											没有账号？前往注册
										</a>
									</div>
								</a-form-item>
					...
					<div v-if="configData.FRONT_BACK_LOGIN_URL_SHOW">
						<a href="/login" class="sys-color-0d84ff">后台登录</a>
					</div>
				</a-card>
			</div>
		</div>
...
const captchaOpen = ref(configData.SYS_BASE_CONFIG.SYSTEM_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C)
	...
	configApi
		...
		.then((data) => {
			...
			if (data) {
				...
				captchaOpen.value = formData.value.SYSTEM_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C
				registerOpen.value = formData.value.SYSTEM_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C
				phoneLogin.value = formData.value.SYSTEM_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C
				emailLogin.value = formData.value.SYSTEM_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C
...
const tenCodeChange = (code) => {
	// 设置新的
	tool.data.set('SYSTEM_TEN_CODE', code)
