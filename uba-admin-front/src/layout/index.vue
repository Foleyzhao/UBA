<template>
	...
	<div class="main-maximize-exit" @click="exitMaximize">
		<fullscreen-exit-outlined class="sys-color-fff"/>
	</div>
...
const init = () => {
	...
	// 获取缓存中的菜单模块是哪个
	const menuModuleId = tool.data.get('SYSTEM_MENU_MODULE_ID')
	if (menuModuleId) {
		// 防止切换一个无此应用的人
...
const getNav = () => {
	...
	nextTick(() => {
		...
		let allWidth = document.querySelector('#sys-line-nav').offsetWidth // 可以显示区域的宽度
...
const handleMouseWheel = (event) => {
	let element = document.querySelector('#sys-line-nav')
...
watch(topHeaderThemeColorOpen, (newValue) => {
	...
	if (layout.value === layoutEnum.TOP) {
		if (newValue) {
			header.classList.add('top-system-header-layout')
			topHeaderMenu.classList.add('top-system-header-layout')
		} else {
			header.classList.remove('top-system-header-layout')
			topHeaderMenu.classList.remove('top-system-header-layout')
		}
	}
})
...
const settingFixedWidth = () => {
	nextTick(() => {
		...
		const showWidth = document.querySelector('.system-tags')
		const mainWidth = document.querySelector('.ant-layout-content')
		if (fixedWidth.value && layout.value === layoutEnum.TOP) {
			breadcrumbWidth?.classList.add(' sys-mg050')
			showWidth?.classList.add(' sys-mg050')
			mainWidth?.classList.add(' sys-pd1180')
		} else {
			breadcrumbWidth?.classList.remove(' sys-mg050')
			showWidth?.classList.remove(' sys-mg050')
			mainWidth?.classList.remove(' sys-pd1180')
		}
	})
}
...
const settingTopHeaderThemeOrColor = (theme, layout) => {
	...
	if (theme === themeEnum.LIGHT && layout === layoutEnum.TOP) {
		header.classList.remove('top-system-header')
		header.classList.add('top-system-header-light')
	} else {
		header.classList.remove('top-system-header-light')
		if (layout === layoutEnum.TOP) {
			header.classList.add('top-system-header')
		} else {
...
const switchoverTopHeaderThemeColor = () => {
	try {
		...
		topHeaderThemeColorOpen.value
			? header.classList.add('system-header-primary-color')
			: header.classList.remove('system-header-primary-color')
		// 判断是否开启了通栏
		const headerLogin = document.getElementById('systemHeaderLogo')
		topHeaderThemeColorSpread.value
			? headerLogin.classList.add('system-header-logo-primary-color')
			: headerLogin.classList.remove('system-header-logo-primary-color')
	...
	if (moduleMenu.value.length > 0) {
		...
		if (menus.length > 0) {
			...
			if (firstMenu.menuType === 'LINK') {
				...
				// 将此模块的唯一值加入缓存
				tool.data.set('SYSTEM_MENU_MODULE_ID', id)
