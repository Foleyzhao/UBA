<template>
	<div class="setting-drawer-index-content">
		<div class="scrollbar">
			...
			<div class="system-setting-checkbox">
				<a-tooltip v-for="(a, i) in sideStyleList" :key="i" placement="top">
					<template #title>
						<span>{{ a.tips }}</span>
					</template>
					<div :class="['system-setting-checkbox-item', a.style]" @click="setSideStyle(a.value)">
						<check-outlined v-if="theme === a.value" class="system-setting-checkbox-item-select-icon"/>
					</div>
				</a-tooltip>
			</div>
			...
			<div class="system-setting-checkbox">
				<a-tooltip v-for="(a, i) in layoutList" :key="i" placement="top">
					...
					<div :class="['system-setting-checkbox-item', a.style]" @click="layoutStyle(a.value)">
						<div class="system-setting-layout-menu-doublerow-inner"/>
						<check-outlined v-if="layout === a.value" class="system-setting-checkbox-item-select-icon"/>
					</div>
				</a-tooltip>
			</div>
			<a-divider/>
			<div class="mb-4">
				...
				<div class="h-[50px]">
					<a-tooltip v-for="(item, index) in colorList" :key="index"
							   class="system-setting-theme-color-colorBlock">
						<template #title>
							<span>{{ item.key }}</span>
						</template>
...
const sideStyleList = ref([
	{
		...
		style: 'system-setting-checkbox-item-dark'
	},
	{
		tips: '亮色主题风格',
		value: themeEnum.LIGHT,
		style: 'system-setting-checkbox-item-light'
	},
	{
		tips: '暗黑模式',
		value: themeEnum.REAL_DARK,
		style: 'system-setting-checkbox-item-realdark'
	}
])
const layoutList = ref([
	{
		...
		style: 'system-setting-layout-menu-classical'
	},
	{
		tips: '双排菜单',
		value: layoutEnum.DOUBLEROW,
		style: 'system-setting-layout-menu-doublerow'
	},
	{
		tips: '顶部菜单',
		value: layoutEnum.TOP,
		style: 'system-setting-layout-menu-top'
	}
])
const sysFormStyleOptions = ref([
	{
		label: '抽屉',
		value: 'drawer'
	},
...
const changeTopHeaderThemeColorOpen = () => {
	...
	if (!topHeaderThemeColorOpen.value) {
		...
		tool.data.set('SYSTEM_TOP_HEADER_THEME_COLOR_SPREAD', false)
	}
}
...
const toggleState = (stateName) => {
	...
	tool.data.set(`SYSTEM_${toolDataName}`, store[stateName])
}
// 设置整体风格主题
const setSideStyle = (value) => {
	store.setTheme(value)
	tool.data.set('SYSTEM_THEME', value)
	layoutChange(layout.value)
}
// 设置整体界面布局
const layoutStyle = (value) => {
	store.setLayout(value)
	tool.data.set('SYSTEM_LAYOUT', value)
	layoutChange(value)
}
// 切换颜色
const tagColor = (value) => {
	tool.data.set('SYSTEM_THEME_COLOR', value)
	store.setThemeColor(value)
}
// 切换表单风格
const formStyleChange = (value) => {
	tool.data.set('SYSTEM_FORM_STYLE', value)
...
onMounted(() => {
	const layout = tool.data.get('SYSTEM_LAYOUT')
...
.system-setting-checkbox-item::before {
	position: absolute;
	top: 0;
	left: 0;
	width: 33%;
...
.system-setting-checkbox-item::after {
	position: absolute;
	top: 0;
	left: 0;
	width: 100%;
...
.system-setting-checkbox-item-light {
	background-color: #ebeef1;
	content: '';
}

.system-setting-checkbox-item-light::before {
	background-color: #fff;
	content: '';
}

.system-setting-checkbox-item-light::after {
	background-color: #fff;
}

.system-setting-checkbox-item-dark {
	z-index: 1;
	background-color: #ebeef1;
	content: '';
}

.system-setting-checkbox-item-dark::before {
	z-index: 1;
	background-color: #001529;
	content: '';
}

.system-setting-checkbox-item-dark::after {
	position: absolute;
	top: 0;
	left: 0;
	width: 100%;
...
.system-setting-checkbox-item-realdark {
	background-color: rgba(0, 21, 41, 0.85);
}

.system-setting-checkbox-item-realdark::before {
	z-index: 1;
	background-color: rgba(0, 21, 41, 0.65);
	content: '';
}

.system-setting-checkbox-item-realdark::after {
	background-color: rgba(0, 21, 41, 0.85);
}

.system-setting-checkbox-item-select-icon {
	position: absolute;
	right: 8px;
	bottom: 8px;
	color: #1677FF;
...
.system-setting-theme-color-colorBlock {
	margin-top: 8px;
	width: 20px;
	height: 20px;
	border-radius: 2px;
...
.system-setting-layout-menu-doublerow {
	z-index: 1;
	background-color: #ebeef1;
	content: '';
}

.system-setting-layout-menu-doublerow::before {
	z-index: 1;
	width: 16%;
	background-color: #001529;
	content: '';
...
.system-setting-layout-menu-doublerow-inner {
	position: absolute;
	top: 0;
	left: 0;
	display: block;
...
.system-setting-layout-menu-doublerow::after {
	position: absolute;
	top: 0;
	left: 0;
	width: 100%;
...
.system-setting-layout-menu-classical {
	z-index: 1;
	background-color: #ebeef1;
	content: '';
}

.system-setting-layout-menu-classical::before {
	z-index: 1;
	background-color: #001529;
	content: '';
}

.system-setting-layout-menu-classical::after {
	position: absolute;
	top: 0;
	left: 0;
	width: 100%;
...
.system-setting-layout-menu-top {
	z-index: 1;
	background-color: #ebeef1;
	content: '';
}

.system-setting-layout-menu-top::before {
	z-index: 1;
	background-color: #ebeef1;
	content: '';
}

.system-setting-layout-menu-top::after {
	z-index: 2;
	position: absolute;
	top: 0;
	left: 0;
