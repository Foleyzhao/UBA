...
import com.huanniankj.mobile.modular.resource.mapper.MobileMenuMapper;
...
public class MobileMenuServiceImpl extends ServiceImpl<MobileMenuMapper, MobileMenu> implements MobileMenuService {
    ...
    public List<Tree<String>> tree(MobileMenuTreeParam mobileMenuTreeParam) {
        ...
        List<TreeNode<String>> treeNodeList = mobileMenuList.stream().map(mobileMenu ->
                        new TreeNode<>(mobileMenu.getId(), mobileMenu.getParentId(), mobileMenu.getTitle(),
                                mobileMenu.getSortCode()).setExtra(JSONUtil.parseObj(mobileMenu)))
    ...
    public void add(MobileMenuAddParam mobileMenuAddParam) {
        ...
        boolean repeatTitle = this.count(new LambdaQueryWrapper<MobileMenu>()
                .eq(MobileMenu::getParentId, mobileMenu.getParentId())
                .eq(MobileMenu::getModule, mobileMenu.getModule())
                .eq(MobileMenu::getCategory, MobileResourceCategoryEnum.MENU.getValue())
    ...
    public void edit(MobileMenuEditParam mobileMenuEditParam) {
        ...
        if (errorLevel) {
            throw new CommonException("不可选择上级菜单：{}",
                    this.getById(originDataList, mobileMenu.getParentId()).getTitle());
        }
    ...
    public void changeModule(MobileMenuChangeModuleParam mobileMenuChangeModuleParam) {
        ...
        List<MobileMenu> mobileMenuList = this.list(new LambdaQueryWrapper<MobileMenu>().eq(MobileMenu::getCategory,
                MobileResourceCategoryEnum.MENU.getValue()));
        List<MobileMenu> mobileMenuChildList =
                this.getChildListById(mobileMenuList, mobileMenu.getId(), true).stream()
                        .peek(mobileMenuTemp ->
                                mobileMenuTemp.setModule(mobileMenuChangeModuleParam.getModule()))
                        .collect(Collectors.toList());
        mobileMenuChildList.forEach(mobileMenuTemp -> {
            boolean repeatTitle = this.count(new LambdaQueryWrapper<MobileMenu>()
                    .eq(MobileMenu::getParentId, mobileMenuTemp.getParentId())
                    .eq(MobileMenu::getModule, mobileMenuTemp.getModule())
                    .eq(MobileMenu::getCategory, MobileResourceCategoryEnum.MENU.getValue())
    ...
    public void delete(List<MobileMenuIdParam> mobileMenuIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(mobileMenuIdList)) {
            ...
            List<MobileMenu> allMenuList = this.list(new LambdaQueryWrapper<MobileMenu>()
                    .in(MobileMenu::getCategory,
                            CollectionUtil.newArrayList(MobileResourceCategoryEnum.MENU.getValue())));
            List<String> toDeleteMenuIdList = CollectionUtil.newArrayList();
            mobileMenuIdList.forEach(menuId -> toDeleteMenuIdList.addAll(
                    this.getChildListById(allMenuList, menuId, true).stream()
    ...
    public List<Tree<String>> menuTreeSelector(MobileMenuSelectorMenuParam mobileMenuSelectorMenuParam) {
        ...
        List<TreeNode<String>> treeNodeList = resourceList.stream().map(mobileMenu ->
                        new TreeNode<>(mobileMenu.getId(), mobileMenu.getParentId(),
                                mobileMenu.getTitle(), mobileMenu.getSortCode())
                                .setExtra(JSONUtil.parseObj(mobileMenu)))
                .collect(Collectors.toList());
        return TreeUtil.build(treeNodeList, "0");
    }
        ...
        mobileMenuList.forEach(mobileMenu -> {
            ...
            if (isLeafMenu) {
                ...
                if (parentNameSplitList.size() > 1) {
                    ...
                    mobileRoleGrantResourceMenuResult.set("title", selfNamePrefix
                            + mobileRoleGrantResourceMenuResult.getStr("title"));
                } else {
                    mobileRoleGrantResourceMenuResult.set("parentName", parentJsonObject.getStr("parentName"));
                }
                mobileRoleGrantResourceMenuResult.set("button", this.getChildListById(mobileButtonList,
                                mobileMenu.getId(), false)
                        .stream().map(tempMobileMenu -> JSONUtil.createObj()
                                .set("id", tempMobileMenu.getId()).set("title", tempMobileMenu.getTitle()))
                        .collect(Collectors.toList()));
    ...
    public List<Tree<String>> loginMobileMenuTree(List<String> menuIdList) {
        ...
        List<MobileMenu> allModuleAndMenuAndSpaList = this.list(new LambdaQueryWrapper<MobileMenu>()
                .in(MobileMenu::getCategory, MobileResourceCategoryEnum.MODULE.getValue(),
                        MobileResourceCategoryEnum.MENU.getValue())
