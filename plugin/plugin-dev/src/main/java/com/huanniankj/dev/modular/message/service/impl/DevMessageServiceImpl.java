package com.huanniankj.dev.modular.message.service.impl;
...
import com.huanniankj.dev.modular.message.enums.DevMessageCategoryEnum;
import com.huanniankj.dev.modular.message.mapper.DevMessageMapper;
import com.huanniankj.dev.modular.message.param.DevMessageIdParam;
import com.huanniankj.dev.modular.message.param.DevMessageListParam;
...
import com.huanniankj.dev.modular.relation.entity.DevRelation;
import com.huanniankj.dev.modular.relation.enums.DevRelationCategoryEnum;
import com.huanniankj.dev.modular.relation.service.DevRelationService;
...
public class DevMessageServiceImpl extends ServiceImpl<DevMessageMapper, DevMessage> implements DevMessageService {
    ...
    public void send(DevMessageSendParam devMessageSendParam) {
        ...
        if (ObjectUtil.isNotEmpty(receiverIdList)) {
            List<String> extJsonList = receiverIdList.stream()
                    .map(userId -> JSONUtil.toJsonStr(JSONUtil.createObj()
    ...
    public Page<DevMessage> page(DevMessagePageParam devMessagePageParam) {
        ...
        if (ObjectUtil.isAllNotEmpty(devMessagePageParam.getSortField(), devMessagePageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(true, devMessagePageParam.getSortOrder()
    ...
    public Page<JSONObject> page(List<String> receiverIdList, String category) {
        if (ObjectUtil.isNotEmpty(receiverIdList)) {
            List<DevRelation> messageRelationList = devRelationService
                    .getRelationListByTargetIdListAndCategory(receiverIdList,
                            DevRelationCategoryEnum.MSG_TO_USER.getValue());
            List<String> messageIdList = messageRelationList.stream().map(DevRelation::getObjectId)
            ...
            if (ObjectUtil.isNotEmpty(messageIdList)) {
                ...
                List<String> relationMessageIdList = CollStreamUtil.toList(messageRelationList,
                        DevRelation::getObjectId);
                ...
                CollectionUtil.sort(jsonObjectList, Comparator.comparingInt(jsonObject ->
                        Convert.toInt(jsonObject.getBool("read"))));
        ...
        if (ObjectUtil.isNotEmpty(devMessageListParam.getReceiverIdList())) {
            List<String> messageIdList = devRelationService
                    .getRelationListByTargetIdListAndCategory(devMessageListParam.getReceiverIdList(),
                            DevRelationCategoryEnum.MSG_TO_USER.getValue()).stream()
                    .filter(devRelation -> JSONUtil
    ...
    public void delete(List<DevMessageIdParam> devMessageIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(devMessageIdList)) {
            // 清除站内信与用户关联关系
            devRelationService.remove(new LambdaUpdateWrapper<DevRelation>().eq(DevRelation::getCategory,
                            DevRelationCategoryEnum.MSG_TO_USER.getValue())
    ...
    public DevMessageResult detail(DevMessageIdParam devMessageIdParam) {
        ...
        DevRelation myMessage = devRelationService.getOne(new LambdaQueryWrapper<DevRelation>()
                .eq(DevRelation::getObjectId, devMessage.getId()).eq(DevRelation::getTargetId,
        ...
        List<DevMessageResult.DevReceiveInfo> receiveInfoList =
                devRelationService.getRelationListByObjectIdAndCategory(devMessage.getId(),
