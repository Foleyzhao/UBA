package com.huanniankj.dev.modular.config.service.impl;
...
import com.huanniankj.dev.modular.config.enums.DevConfigCategoryEnum;
import com.huanniankj.dev.modular.config.mapper.DevConfigMapper;
...
public class DevConfigServiceImpl extends ServiceImpl<DevConfigMapper, DevConfig> implements DevConfigService {
    ...
    public Page<DevConfig> page(DevConfigPageParam devConfigPageParam) {
        ...
        if (ObjectUtil.isAllNotEmpty(devConfigPageParam.getSortField(), devConfigPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(true, devConfigPageParam.getSortOrder()
    ...
    public List<DevConfig> sysBaseList() {
        ...
        // 或key为特殊的几个
        lambdaQueryWrapper.or().in(DevConfig::getConfigKey,
                CollectionUtil.newArrayList(SYSTEM_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_B_KEY,
                SYSTEM_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C_KEY,
                SYSTEM_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_B_KEY,
                SYSTEM_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C_KEY,
                SYSTEM_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_B_KEY,
                SYSTEM_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C_KEY));
    ...
    public void delete(List<DevConfigIdParam> devConfigIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(devConfigIdList)) {
            ...
            if (ObjectUtil.isNotEmpty(devConfigList)) {
                ...
                if (devConfigResultList.size() != 1 || !devConfigResultList.get(0)
                        .equals(DevConfigCategoryEnum.BIZ_DEFINE.getValue())) {
