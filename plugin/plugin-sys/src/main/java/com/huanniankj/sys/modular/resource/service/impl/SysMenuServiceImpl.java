...
import com.huanniankj.sys.modular.resource.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public Page<SysMenu> page(SysMenuPageParam sysMenuPageParam) {
        ...
        if (ObjectUtil.isAllNotEmpty(sysMenuPageParam.getSortField(), sysMenuPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(true, sysMenuPageParam.getSortOrder()
    ...
    private void fillParentSysMenuInfo(List<SysMenu> resourceList) {
        if (CollUtil.isNotEmpty(resourceList)) {
            List<SysMenu> parentRelationSysMenus = resourceList.stream().filter(distinctByKey(SysMenu::getParentId))
                    .collect(Collectors.toList());
    ...
    public void add(SysMenuAddParam sysMenuAddParam) {
        ...
        boolean repeatTitle = this.count(new LambdaQueryWrapper<SysMenu>()
                .eq(SysMenu::getParentId, sysMenu.getParentId())
                .eq(SysMenu::getModule, sysMenu.getModule())
        ...
        CommonDataChangeEventCenter.doAddWithData(SysDataTypeEnum.RESOURCE.getValue(),
                JSONUtil.createArray().put(sysMenu));
    }
    ...
    public void edit(SysMenuEditParam sysMenuEditParam) {
        ...
        boolean errorLevel = this.getChildListById(originDataList, sysMenu.getId(), true).stream()
                .map(SysMenu::getId).toList().contains(sysMenu.getParentId());
        if (errorLevel) {
            throw new CommonException("不可选择上级菜单：{}", this.getById(originDataList,
                    sysMenu.getParentId()).getName());
        }
    ...
    public void changeModule(SysMenuChangeModuleParam sysMenuChangeModuleParam) {
        ...
        List<SysMenu> sysMenuChildList = this.getChildListById(sysMenuList, sysMenu.getId(), true).stream()
                .peek(sysMenuTemp -> sysMenuTemp.setModule(sysMenuChangeModuleParam.getModule()))
                .collect(Collectors.toList());
        sysMenuChildList.forEach(sysMenuTemp -> {
            boolean repeatTitle = this.count(new LambdaQueryWrapper<SysMenu>()
                    .eq(SysMenu::getParentId, sysMenuTemp.getParentId())
                    .eq(SysMenu::getModule, sysMenuTemp.getModule())
                    .eq(SysMenu::getCategory, SysResourceCategoryEnum.MENU.getValue())
    ...
    public void delete(List<SysMenuIdParam> sysMenuIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(sysMenuIdList)) {
            ...
            sysMenuIdList.forEach(menuId -> toDeleteMenuIdList.addAll(this.getChildListById(allMenuList,
                            menuId, true).stream()
                    .map(SysMenu::getId).toList()));
            if (ObjectUtil.isNotEmpty(toDeleteMenuIdList)) {
                // 清除对应的角色与资源信息
                sysRelationService.remove(new LambdaUpdateWrapper<SysRelation>()
                        .in(SysRelation::getTargetId, toDeleteMenuIdList)
