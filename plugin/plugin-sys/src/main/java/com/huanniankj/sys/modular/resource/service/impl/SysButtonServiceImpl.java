...
import com.huanniankj.sys.modular.resource.mapper.SysButtonMapper;
...
public class SysButtonServiceImpl extends ServiceImpl<SysButtonMapper, SysButton> implements SysButtonService {
    ...
    public Page<SysButton> page(SysButtonPageParam sysButtonPageParam) {
        ...
        if (ObjectUtil.isAllNotEmpty(sysButtonPageParam.getSortField(), sysButtonPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(true, sysButtonPageParam.getSortOrder()
    ...
    public void add(SysButtonAddParam sysButtonAddParam) {
        ...
        CommonDataChangeEventCenter.doAddWithData(SysDataTypeEnum.RESOURCE.getValue(),
                JSONUtil.createArray().put(sysButton));
    }
    ...
    public void addForGenButton(String menuId, String className, String functionName) {
        ...
        CollectionUtil.newArrayList(JSONUtil.createObj().set("title", "新增" + functionName)
                        .set("code", classNameFirstLower + "Add").set("sortCode", 1),
                JSONUtil.createObj().set("title", "编辑" + functionName).set("code", classNameFirstLower + "Edit")
                        .set("sortCode", 2),
                JSONUtil.createObj().set("title", "删除" + functionName).set("code", classNameFirstLower + "Delete")
                        .set("sortCode", 3),
                JSONUtil.createObj().set("title", "批量删除").set("code", classNameFirstLower + "BatchDelete")
                        .set("sortCode", 4),
                JSONUtil.createObj().set("title", "导入" + functionName).set("code", classNameFirstLower + "Import")
                        .set("sortCode", 5),
                JSONUtil.createObj().set("title", "导出" + functionName).set("code", classNameFirstLower + "Export")
                        .set("sortCode", 6)).forEach(jsonObject -> {
    ...
    public void delete(List<SysButtonIdParam> sysButtonIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(buttonIdList)) {
            // 获取按钮的父菜单id集合
            List<String> parentMenuIdList = sysMenuService.list(new LambdaQueryWrapper<SysMenu>()
                            .in(SysMenu::getId, buttonIdList)
                            .eq(SysMenu::getCategory, SysResourceCategoryEnum.BUTTON.getValue()))
                    .stream().map(SysMenu::getParentId)
                    .collect(Collectors.toList());
            if (ObjectUtil.isNotEmpty(parentMenuIdList)) {
                sysRelationService.list(new LambdaQueryWrapper<SysRelation>()
                        .in(SysRelation::getTargetId, parentMenuIdList)
                    ...
                    if (ObjectUtil.isNotEmpty(buttonInfoList)) {
                        Set<String> intersectionDistinct =
                                CollectionUtil.intersectionDistinct(buttonIdList, buttonInfoList);
                        if (ObjectUtil.isNotEmpty(intersectionDistinct)) {
                            Collection<String> disjunction =
                                    CollectionUtil.disjunction(buttonInfoList, intersectionDistinct);
                            extJsonObject.set("buttonInfo", disjunction);
                        }
                    }
                    // 清除对应的角色与资源信息中的【授权的按钮信息】
                    sysRelationService.update(new LambdaUpdateWrapper<SysRelation>()
                            .eq(SysRelation::getId, sysRelation.getId())
