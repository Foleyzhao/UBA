...
import com.huanniankj.biz.core.enums.BizBuildInEnum;
...
import com.huanniankj.biz.modular.user.enums.BizRoleCategoryEnum;
import com.huanniankj.biz.modular.user.enums.BizUserStatusEnum;
import com.huanniankj.biz.modular.user.mapper.BizUserMapper;
...
import com.huanniankj.biz.modular.user.result.BizUserExportResult;
...
import com.huanniankj.sys.api.SysUserApi;
...
public class BizUserServiceImpl extends ServiceImpl<BizUserMapper, BizUser> implements BizUserService {
    ...
    public Page<BizUser> page(BizUserPageParam bizUserPageParam) {
        ...
        if (ObjectUtil.isNotEmpty(bizUserPageParam.getSearchKey())) {
            queryWrapper.lambda().and(q -> q.like(BizUser::getAccount,
                            bizUserPageParam.getSearchKey())
        ...
        if (ObjectUtil.isAllNotEmpty(bizUserPageParam.getSortField(), bizUserPageParam.getSortOrder())) {
            ...
            queryWrapper.orderBy(true, bizUserPageParam.getSortOrder()
    ...
    public void edit(BizUserEditParam bizUserEditParam) {
        ...
        // 发布更新事件
        CommonDataChangeEventCenter.doUpdateWithData(BizDataTypeEnum.USER.getValue(),
                JSONUtil.createArray().put(bizUser));
    }
    ...
    public void delete(List<BizUserIdParam> bizUserIdParamList) {
        ...
        if (ObjectUtil.isNotEmpty(bizUserIdList)) {
            ...
            // 获取这些人员的的机构id集合
            Set<String> userOrgIdList = this.listByIds(bizUserIdList).stream().map(BizUser::getOrgId)
            ...
            // 清除【将这些人员作为主管】的信息
            this.update(new LambdaUpdateWrapper<BizUser>().in(BizUser::getDirectorId, bizUserIdList)
                    .set(BizUser::getDirectorId, null));
            ...
            bizOrgService.update(new LambdaUpdateWrapper<BizOrg>().in(BizOrg::getDirectorId, bizUserIdList)
                    .set(BizOrg::getDirectorId, null));
    ...
    public void disableUser(BizUserIdParam bizUserIdParam) {
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            if (!loginUserDataScope.contains(bizUser.getOrgId())) {
                throw new CommonException("您没有权限禁用该机构下的人员：{}，机构id：{}", bizUser.getName(),
                        bizUser.getOrgId());
            }
        } else {
    ...
    public void enableUser(BizUserIdParam bizUserIdParam) {
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            if (!loginUserDataScope.contains(bizUser.getOrgId())) {
                throw new CommonException("您没有权限启用该机构下的人员：{}，机构id：{}", bizUser.getName(),
                        bizUser.getOrgId());
            }
        } else {
            if (!bizUser.getId().equals(StpUtil.getLoginIdAsString())) {
    ...
    public void resetPassword(BizUserIdParam bizUserIdParam) {
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            if (!loginUserDataScope.contains(bizUser.getOrgId())) {
                throw new CommonException("您没有权限为该机构下的人员：{}重置密码，机构id：{}", bizUser.getName(),
                        bizUser.getOrgId());
            }
        } else {
            if (!bizUser.getId().equals(StpUtil.getLoginIdAsString())) {
    ...
    public void grantRole(BizUserGrantRoleParam bizUserGrantRoleParam) {
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            if (!loginUserDataScope.contains(bizUser.getOrgId())) {
                throw new CommonException("您没有权限为该机构下的人员：{}授权角色，机构id：{}", bizUser.getName(),
                        bizUser.getOrgId());
            }
        } else {
            if (!bizUser.getId().equals(StpUtil.getLoginIdAsString())) {
        ...
        try {
            ...
            if (ObjectUtil.isNotEmpty(bizUserExportParam.getUserIds())) {
                ...
                if (ObjectUtil.isNotEmpty(bizUserExportParam.getSearchKey())) {
                    queryWrapper.lambda().and(q -> q.like(BizUser::getAccount,
                                    bizUserExportParam.getSearchKey())
            ...
            String fileName = "系统B端人员信息清单.xlsx";
            ...
            EasyExcel.write(tempFile.getPath(), BizUserExportResult.class)
                    ...
                    // 机构分组合并单元格
                    .registerWriteHandler(new CommonExcelCustomMergeStrategy(bizUserExportResultList.stream()
                            .map(BizUserExportResult::getGroupName)
    ...
    public void exportUserInfo(BizUserIdParam bizUserIdParam, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (ObjectUtil.isNotEmpty(bizUser.getBirthday())) {
                try {
                    // 年龄
                    long age = DateUtil.betweenYear(DateUtil.parseDate(bizUser.getBirthday()), DateTime.now(),
                            true);
                    if (age != 0) {
                        map.put("age", age + "岁");
                    }
            ...
            // 生成临时导出文件
            resultFile = FileUtil.file(FileUtil.getTmpDir() + File.separator + "系统B端人员信息_"
                    + bizUser.getName() + ".docx");
            // 写入
    ...
    public List<Tree<String>> orgTreeSelector() {
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            ...
            loginUserDataScope.forEach(orgId -> bizOrgSet.addAll(
                    bizOrgService.getParentListById(allOrgList, orgId, true)));
        ...
        if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
            if (ObjectUtil.isNotEmpty(bizUserSelectorRoleParam.getOrgId())) {
                if (loginUserDataScope.contains(bizUserSelectorRoleParam.getOrgId())) {
                    return BeanUtil.toBean(sysRoleApi.roleSelector(bizUserSelectorRoleParam.getOrgId(),
                            bizUserSelectorRoleParam.getCategory(), bizUserSelectorRoleParam.getSearchKey(),
                            loginUserDataScope, true), Page.class);
                } else {
                    return new Page<>();
                }
            } else {
                if (ObjectUtil.isNotEmpty(bizUserSelectorRoleParam.getCategory())
                        & BizRoleCategoryEnum.GLOBAL.getValue().equals(bizUserSelectorRoleParam.getCategory())) {
                    // 查询系统级别的
                    return BeanUtil.toBean(sysRoleApi.roleSelector(null, bizUserSelectorRoleParam.getCategory(),
                            bizUserSelectorRoleParam.getSearchKey(), null, true),
                            Page.class);
                } else {
                    return BeanUtil.toBean(sysRoleApi.roleSelector(null, bizUserSelectorRoleParam.getCategory(),
                            bizUserSelectorRoleParam.getSearchKey(), loginUserDataScope, true),
                            Page.class);
                }
            }
        } else {
